...
********************************************************************************
        ...
        The allcatplot Stata program does not plot all or any cat(s) on a graph. Instead, it ensures that all predefined response categories for a variable, includi
> ng those that were not selected, are included in a graph. Traditional plotting methods omit such unselected categories, potentially skewing interpretation. allcat
> plot uses the predefined value labels to identify and label such omitted response options and add them to the graph as zero-height bars. Additionaly, it also supp
> orts custom lists, labels and graph customization directly through the program syntax.

        This program is especially useful for surveys and assessments utilizing likert scales or other structured response options, where it offers a holistic view 
> of all potential responses in the graph, including those that weren't selected, thereby ensuring a comprehensive understanding of the full range of response optio
> ns.

        Additionally, allcatplot can be used to do the opposite of what its name suggests: exclude specific categories from the graph for clarity or analytical purp
> oses. For instance, in a dataset with varied selections across options A, B, C, and D for a specific variable, the program can be configured to display only the r
> esponses for options A, B, and C (or any other combination of the four in any order of your choice). This doesn't eliminate the data for option D but merely hides
>  it from the graph, with the displayed bar heights accurately reflecting the frequencies or percentages of responses for the displayed options as per the total co
> unt.
...
qui {   
        ...
        ********************************************************************************
        ...
        /// locals for N and missing obs
                ...
                /// the locals for the number of observations and missing have to be calculated prior to using marksample as marksample correctly drops all missing 
> values of varlist
    ...
    local var_lab `varlist'
        ...
        ********************************************************************************
        ...
        /// now, we move on to the values that will be mapped. if the user has requested for a default list of values, that request will be prioritized and will alr
> eady be stored as a local called 'list'
   ...
   /// Extract the ith label and list value
    ...
    label values `varlist' `val_lab'
        ...
        ********************************************************************************
        ...
        /// this additional step is necessary because the varlist may have negative values and the negative sign (-) isn't allowed as part of a var name in Stata. I
> dentified as a potential issue by Prabhmeet Kaur. 
